...
const KIND = { PALM: 1, TREE: 2, BANANA: 3, SHRUB: 4, YOUNG: 5, FERN: 6 };
...
export function scatterVegetation( site, seed = 99 ) {
	...
	const out = { palms: [], trees: [], bananas: [], shrubs: [], youngPalms: [], ferns: [] };
	const TAU = Math.PI * 2;
	const c = {};
...
export function buildGrassMask( site ) {
	...
	for ( let j = 1; j < res - 1; j ++ ) {
		...
		for ( let i = 1; i < res - 1; i ++ ) {
			...
			if ( t.heightAt( x, z ) < 1.85 ) continue;
			...
			// dune grass: back of the beach and dunes
			let dune = 0;
			if ( bay ) dune = smoothstep( 2.0, 2.4, c.h ) * ( 1 - smoothstep( 4.8, 6.5, c.h ) ) * smoothstep( - 0.12, 0.42, clump ) * ( 0.4 + 0.6 * smoothstep( 0.2, 0.6, c.sand ) );

			// sea oats: fore-dunes
			let oats = 0;
			if ( bay ) oats = smoothstep( 2.25, 2.6, c.h ) * ( 1 - smoothstep( 3.8, 4.6, c.h ) ) * smoothstep( 0.0, 0.45, N.noise( x / 13 + 3.1, z / 13 - 7.7 ) );

			// beach creeper: upper beach runners
			let vine = 0;
			if ( bay ) vine = smoothstep( 1.95, 2.15, c.h ) * ( 1 - smoothstep( 3.0, 3.7, c.h ) ) * smoothstep( 0.0, 0.4, N2.noise( x / 11 - 2.3, z / 11 + 5.3 ) );
