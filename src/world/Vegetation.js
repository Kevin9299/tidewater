...
//
// Draw calls (main pass): palm near, palm far, understory (young palms + bananas + ferns),
// canopy near (trees + shrubs), canopy impostors (octahedral, trees + shrubs), grass near,
// grass far = 7 (fewer when a level is empty). Shadow casters: palm near, canopy near.
//
...
export class Vegetation {
	...
	constructor( { scene, terrain, village = null } ) {
		...
		// understory: young palms, bananas and ferns in one mesh; the plant kind rides on the seed
		const underRecs = [
			...recs.youngPalms.map( ( r ) => ( { ...r, seed: UNDERSTORY.YOUNG + r.seed * 0.999, qr: UNDER_FADE[ 1 ] + 10 } ) ),
			...recs.bananas.map( ( r ) => ( { ...r, seed: UNDERSTORY.BANANA + r.seed * 0.999, qr: UNDER_FADE[ 1 ] + 10 } ) ),
