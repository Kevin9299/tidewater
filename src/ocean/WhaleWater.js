...
import { Fn, float, vec2, uniformArray, int, Loop, length, smoothstep, sin, max } from 'three/tsl';
...
const N = 6;
export const whaleMarks = uniformArray( new Array( N ).fill( 0 ).map( () => new THREE.Vector4( 0, 0, 1, 0 ) ), 'vec4' );

// foam and slick amounts at a surface point (TSL)
export const whaleWater = Fn( ( [ xz ] ) => {

	const foam = float( 0 ).toVar(), slick = float( 0 ).toVar();
	Loop( N, ( { i } ) => {

		const e = whaleMarks.element( int( i ) );
		const d = length( xz.sub( e.xy ) ).div( e.z );
		const core = float( 1 ).sub( smoothstep( 0.2, 1.0, d ) );
		// churned water breaks up into lumps and streaks with dark water between them, denser
		// toward the middle
		const q = xz.mul( 0.8 ), q2 = xz.mul( 2.9 );
		const n1 = sin( q.x.add( sin( q.y.mul( 1.3 ) ).mul( 1.7 ) ) ).mul( sin( q.y.mul( 1.1 ).add( sin( q.x.mul( 1.7 ) ).mul( 1.4 ) ) ) );
		const n2 = sin( q2.x.mul( 1.1 ).add( sin( q2.y ).mul( 1.3 ) ) ).mul( sin( q2.y.mul( 0.9 ).add( sin( q2.x.mul( 1.2 ) ) ) ) );
		const lumps = smoothstep( - 0.25, 0.55, n1.mul( 0.7 ).add( n2.mul( 0.45 ) ).add( float( 1 ).sub( d ).mul( 0.5 ) ) );
		foam.assign( max( foam, core.mul( max( e.w, 0 ) ).mul( lumps ).mul( 0.85 ) ) );
		slick.assign( max( slick, core.mul( max( e.w.negate(), 0 ) ) ) );

	} );
	return vec2( foam, slick );
...
export class WhaleWater {
	...
	burst( whale, spray, p, r, up, n ) {
		...
		for ( let k = 0; k < 9; k ++ ) {
			...
			const q = new THREE.Vector3( p.x + Math.cos( a ) * r * ( 0.4 + Math.random() * 0.6 ), water + 0.2, p.z + Math.sin( a ) * r * ( 0.4 + Math.random() * 0.6 ) );
			const vel = new THREE.Vector3( Math.cos( a ) * 2.5, up * ( 0.6 + Math.random() * 0.5 ), Math.sin( a ) * 2.5 );
